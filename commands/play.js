...
.setColor(COLOR);
    ...
    if (!videoPattern.test(args[0]) && playlistPattern.test(args[0])) {
      embed.setAuthor(" Error")
      embed.setDescription ("<a:ss_bhagoo:741157424978722976> I am Unable To Play Playlist for now")
      return message.channel.send(embed);
    }
    ...
    if (urlcheck) {
      try {
        ...
        if (message.include === "copyright") {
          return message
            .reply("<a:ss_bhagoo:741157424978722976> THERE IS COPYRIGHT CONTENT IN VIDEO -_-")
            .catch(console.error);
        } else {
          console.error(error);
        }
      ...
      try {
        ...
        if(error.errors[0].domain === "usageLimits") {
          return message.channel.send(" <a:ss_Hunter:742982140672606300> Your YT API limit is over and it will be restored under 24 hours")
        }
      }
    }
    ...
    if (serverQueue) {
      ...
      embed.setDescription(`<a:ss_Hunter:742982140672606300> **[${song.title}](${song.url})**`)
